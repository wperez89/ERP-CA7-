...
import { MotivoSalidaVeh, conductor, estadoSolVeh, pasajerosVeh } from 'src/app/models/vehiculos/tablasVehiculo.models';
...
export class NewSolicitudComponent implements OnInit {
  ...
  constructor(private fb:FormBuilder, private vehiculoService:VehiculoService, private archivoService:ArchivoDigitalService,
    private usuarioservice:UsuarioService, private router:Router, private activatedRoute: ActivatedRoute,
    private modalService: NgbModal,private personaService:PersonasService, private sharedVeh:SharedVehService)
   { 
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.subscribe((params)=>
    {
      if(params['num_sol']&&params['estado']&&params['id_solic'])
      {
        ...
        this.idSolicitante = params['id_solic'];
        this.cargarEstados();
        this.cargarSolicitudID(this.idEstado,this.idSolicitud,this.idSolicitante);
      }
    })
  ...
  {
    this.cargando = true;
    this.vehiculoService.cargarEstadosTodos()
    .subscribe((resp:any)=>
    {
      //console.log(resp);
      this.estados = resp;
  ...
  {
    this.cargando = true;
    this.vehiculoService.cargarMotivoSalidaVeh()
    .subscribe((resp:any)=>
    {
      //console.log(resp);
  ...
  {
    this.cargando = true;
    this.vehiculoService.cargarSolicitudID(estado,id,solicitante).subscribe
        ((resp:any)=>
        {
          //console.log(resp);
          let solicitud = resp[0];
          const fechaRegreso = solicitud.FECHA_REGRESO.substring(0, solicitud.FECHA_REGRESO.lastIndexOf('.'));
          const fechaRegresoFormateada = fechaRegreso.replace('T', ' ');
          const fechaSalida = solicitud.FECHA_SALIDA.substring(0, solicitud.FECHA_SALIDA.lastIndexOf('.'));
  ...
  {
    {
      ...
      this.vehiculoService.crearSolicitud(this.solicitudForm.value)
      .subscribe((resp:any)=>{
  ...
  mostrarPersona()
  {
    ...
    this.personaService.getNombresPersonabyCedula(this.idPasajero)
    .subscribe((resp:any)=>
    {
